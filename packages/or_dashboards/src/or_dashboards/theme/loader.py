...
On import:
  ...
  2. If env var ``OR_DASHBOARDS_PROJECT_ROOT`` is set and the project has its
...
Other blocks import from ``or_dashboards.theme`` (the package);
the package's ``__init__.py`` re-exports the constants defined here.

The env var must be set BEFORE any ``or_dashboards`` import — the CLI does
...
_project_root = os.environ.get("OR_DASHBOARDS_PROJECT_ROOT")
if _project_root:
