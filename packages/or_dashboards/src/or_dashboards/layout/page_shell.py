...
from ``or_dashboards.theme`` (sourced from ``theme.yaml``).
Chrome behaviour flags (sidebar enabled, sidebar position) come from
``or_dashboards.layout.loader`` (sourced from ``layout.yaml``).
"""
from dash import html

from or_dashboards.layout.loader import SIDEBAR_ENABLED, SIDEBAR_POSITION
