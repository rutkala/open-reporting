"""
Ingests Polish voivodship labour market data from GUS BDL API into PostgreSQL.

Variables pulled (all at voivodship / NUTS-2 level):
  60270  - Stopa bezrobocia rejestrowanego (%) — registered unemployment rate
  64428  - Przeciętne miesięczne wynagrodzenia brutto (PLN) — avg monthly gross wages
  458421 - Produkt krajowy brutto na 1 mieszkańca (PLN) — GDP per capita

Usage:
  BDL_API_KEY=your_key python ingestion/labour_ingest.py
"""
...
VARIABLES = {
    "unemployment_rate": 60270,
    "avg_wages":         64428,
    "gdp_per_capita":    458421,
}
...
def setup_schema(cur):
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS raw.bdl_labour (
...
def upsert_rows(cur, variable_name, rows):
    if not rows:
        return 0
    records = [
        (r["variable_id"], variable_name, r["unit_id"], r["unit_name"],
         r["year"], r["value"], r["flag"])
        for r in rows
    ]
    execute_values(
        cur,
        """
        INSERT INTO raw.bdl_labour
...
def main():
    log.info("Starting GUS BDL labour market ingestion")
    if not API_KEY:
