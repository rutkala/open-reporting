...
or a dashboard imports from `products/ingestion/` or `products/warehouse/`).
→ Violates module boundary. The shared library between platform and products is
  the `dbr` package (`packages/dbr/src/dbr/`) only.
...
| What is the shared library between platform and products? | `dbr` package (`packages/dbr/src/dbr/`) only | BLOCK-06 |
