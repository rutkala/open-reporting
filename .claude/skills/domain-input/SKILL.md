...
Produced by: `/research` (invoked by `/document`)
Consumed by: `/document`, `/design`, any domain-specific product skill
...
When `/research` is invoked for domain research (by `/document`), follow these steps:
