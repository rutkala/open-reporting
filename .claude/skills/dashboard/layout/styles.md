...
## What it is
The complete inline style dictionary and layout constants for a dashboard's `app.py`.
Copy this verbatim — it is identical for every dashboard (only colour tokens change,
and those come from `theme.py` imports).

Source of truth: `products/dashboards/template/app.py` lines 140–242.

---

## Layout constants (define before S)

```python
SIDEBAR_W         = "240px"
SIDEBAR_COLLAPSED = "44px"
GAP    = "4px"    # gap between sidebar and main area, and page edge
RADIUS = "10px"   # sidebar border radius
```

---

## Complete S dict (copy verbatim)
...
## Rules
- Copy this block verbatim — do not paraphrase or omit keys
- All colour references (`BORDER`, `TEXT`, `SUBTEXT`, `BG_PAGE`, `BG_SURFACE`) must be imported from `theme.py` before `S` is defined
- `GAP` and `RADIUS` must be defined as constants before `S` (they are used in f-strings inside `S`)
- Do not add inline styles to layout components directly — put them in `S` and reference by key
- `"group"` and `"group-title"` are for optional sub-headings within a section — use only when a section has distinct named sub-groups
