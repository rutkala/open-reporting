...
- `PYTHONPATH` includes `/opt/open-reporting/.claude/skills` so the dashboard can
  import `complex_dashboard.assets.pages.*` and `complex_dashboard.assets.deploy.*`
...
- `PYTHONPATH` must include **both** `/opt/open-reporting` and `/opt/open-reporting/.claude/skills` — the second entry lets the dashboard import `complex_dashboard.assets.pages.layout.styles`, `complex_dashboard.assets.deploy.app_init`, etc.
