"""
Showroom — sample data loaders.

Powers the visual-component showroom in ``showroom.py``. Each chart in
the showroom binds to one of these loaders so a domain dashboard can
swap them for warehouse-backed equivalents without touching the
``MEASURES`` registry, the chart calls, or the page assembly.

Structure mirrors what a domain semantic_service.py would provide:

- Raw loaders (``load``, ``load_geo``, …) — source data, used internally.
- Aggregated loaders (``load_by_year``, ``load_by_category``,
  ``load_by_period``) — pre-grouped DataFrames consumed directly by chart
  calls.
- ``load_scalars()`` — single values for KPI cards.

When building a domain dashboard, replace these loaders with calls to
your domain's ``semantic_service.py`` that execute SQL against the
warehouse. ``measures.py`` and ``showroom.py`` stay unchanged.
