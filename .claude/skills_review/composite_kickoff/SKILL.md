...
Run `/composite_feasibility {issue ID}` — spawns architecture-critic, analytical-validator, and cost-estimator in parallel against the issue description. This runs silently; surface findings in Step 4.
...
### Feasibility
{FEASIBLE / PARTIAL / BLOCKED — from /composite_feasibility output}
...
### Proposed approach
{"/basic_research [topic]" if approach is unclear, or "/composite_plan [task]" if ready to design}
```
...
Run `/composite_develop {issue ID}` — this drives the full pipeline:
...
2. `/composite_plan` — design the solution, present for approval before any code
3. Create feature branch, write code (`data-engineer` for `platform/`, `ops-engineer` for infra)
4. `/composite_review` — auto-commits, pushes, opens PR when clean; loops to fix when blocked
