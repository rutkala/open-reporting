...
name: composite_test
description: >
  Test a completed product against its requirements document. Use when /composite_evaluate invokes
  ...
  must be verified — nothing is assumed. Failures return to /composite_build with an exact list.
  Triggers when: "run tests", "verify the product works", "check acceptance criteria",
  or when /composite_evaluate reaches the functional testing step for a code product.
...
Every acceptance criterion must be verified. Failures return to `/composite_build` with a specific list.
...
- PASS → proceeds to `/composite_release`
- FAIL → returns to `/composite_build` with specific failure list
...
5. If any FAIL: return to `/composite_build` with specific failure list; re-run after fixes
6. If all PASS: proceed to `/composite_release`
...
**On failure**
Return to `/composite_build` with:
