...
description: >
  ...
  Invoked by /composite_develop as Step 1, or standalone when producing a knowledge base, requirements
  ...
  "research and document X", or when /composite_develop reaches Step 1.
user-invocable: true
...
### 1. Brainstorm
Invoke `/basic_brainstorm` to surface angles, source types, and open questions before collecting anything.
...
### 3. Analyze
Invoke `/basic_analyze` to assess collected material before writing.
...
- Identifies gaps: minor ones go into the artifact's Gaps section; major ones trigger another `/basic_collect` pass

### 4. Save
Invoke `/basic_save` to synthesise collected material into the artifact and write it to disk.
