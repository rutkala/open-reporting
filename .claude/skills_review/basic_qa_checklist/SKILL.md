...
A QA checklist is the structured output of the `/composite_evaluate` step. It records the result
of testing every acceptance criterion and aggregates evaluator agent findings into
a single PASS/FAIL verdict.

Produced by: `/composite_evaluate` (via `/write`)
Consumed by: `/composite_release` (PASS required to proceed)

---

## Location

`products/domain-briefs/{domain}/basic_qa_checklist.md`
