...
- **`packages/dbr/` changes do NOT auto-deploy — and 200 OK does NOT prove they did.** dbr is editable-installed: a live `or-<domain>.service` keeps running the framework code it booted with until restarted. A `curl` 200 only proves *a* process answered, not that it runs your *new* code — stale services pass it. This is the #1 cause of "reported resolved but the dashboard never changed." **After ANY edit under `packages/dbr/`, commit first, then run `python3 infra/scheduler/redeploy_dashboards.py`.** It restarts all 16 services and polls each live page until its `<meta name="dbr-build">` stamp equals repo HEAD. It exits non-zero and prints STALE/DOWN per dashboard if any service is not on HEAD — **if it exits non-zero, the work is NOT resolved; do not report success.** Commit dbr code BEFORE redeploying so the stamp target (HEAD) includes your change; if you commit again after, redeploy again or the stamp will trail HEAD.
- **End-to-end verify, not just 200 OK.** After `dbr run` (single dashboard) or `redeploy_dashboards.py` (fleet): confirm the response is the Dash app, and for layout/visual changes take a Playwright screenshot — `curl` cannot see rendered layout. The build-SHA stamp proves *which code* is live; the screenshot proves it *looks right*.
